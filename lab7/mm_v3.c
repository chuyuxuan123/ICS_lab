...
// Given block ptr bp of a free block, compute address of previous/next free block
...
static char* blk_headp = NULL;                 // A pointer that always pointing to the first free block
int flag;
 ...
 * This function initializes the heap, gets four words from system and put the padding word, prologue block and epilogue block on.
 * Then it set heap_listp to the prologue block, blk_headp to NULL, and extend the heap.
 * If there is a problem in initialization, it stops and returns -1, otherwise it finish initialization and return 0.
 */
int mm_init(void)
{
...
/*
 * remove_node - remove a free block from the free list since it's allocated or being coalesced
 */
...
/*
 * add_node - add a free block to the top of the free list
...
static void* coalesce(void* bp)
{
    ...
    if(prev_alloc && !next_alloc)              // Case 2 (case 1 only need add_node())
    {
...
/*
 * mm_free - Free a block and uses boundary-tag coalescing to merge it with and adjacent free blocks in constant time
 */
...
/*
 * mm_realloc - Reallocate the allocated block to a block whose capacity is size bytes. Return the block pointer of the new block.
 */
...
int mm_check(void)
{
    ...
    for(fp = blk_headp; NULL != fp; fp = NEXT_FREE(fp))
    {
        if(GET_ALLOC(fp))
        {
    ...
    for(fp = blk_headp; NULL != fp; fp = NEXT_FREE(fp))
    {
        if(0 != GETADDR(PRED(blk_headp)))
        {
            fprintf(stderr, "invalid blk_headp predecessot pointer");
            return 0;
        }
