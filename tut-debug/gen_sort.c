...
enum { recno_len = 10 };
enum { pad_len = 80 };
enum { rec_len = key_len + recno_len +  pad_len };//-----------------bug: the leength of each rec

enum { buf_nrec = 1 << 4 };
enum { buf_size = buf_nrec * 100 };//----------------------------bug forget ;

typedef struct record{ 
	char key[key_len] ;
	char recno[recno_len];
	char pad[pad_len];
}record_t;//-------------------------------------------bug1 here: forget ;
 ...
 */
static void 
gen_rec(record_t *rec)//----------------------------bug2 record_t
{
...
main (int argc, char *argv[])
{
	...
	/* prepare buf */
	char *buf = (char *)malloc(buf_size) ;//bug : previous:buf_nerc
	if (buf  == NULL) {
		printf("error: unable to malloc buffer\n");
		exit(EXIT_FAILURE);
	}

	FILE *file = fopen(fn, "w");//--------------------bug :file mode write
	if (!file) {//--------------------forget {
