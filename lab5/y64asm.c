...
int add_symbol(char *name)
{
    ...
    while (temp->next != NULL)
    {
        ...
        {
            return -1; // error : name has already exist
        }
    }

    /* create new symbol_t (don't forget to free it)*/
    symbol_t *new_sym = (symbol_t *)malloc(sizeof(symbol_t)); //free in finit()
    new_sym->name = name;
    new_sym->addr = vmaddr;
    new_sym->next = NULL;
...
#define IS_REG(s) (*(s) == '%')
#define IS_IMM(s) (*(s) == '$')
#define IS_HEX(s) ((*(s) >= 'a' && *(s) <= 'f') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) == 'x'))
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
    const reg_t *temp_reg = find_register(*ptr);
    if (temp_reg == NULL)
    {
        return PARSE_ERR;
    }
...
*           call SUM  #invoke SUM function */
    ...
    if (parse_instr(&ptr, &inst) == PARSE_INSTR)
    {
        ...
        (line->y64bin).codes[0] = inst->code; // write the first byte of inst
        ...
        */
        ...
        /*
        case rrmov, alu: 
        only parse two registers
        */
        case I_RRMOVQ:
        case I_ALU:
        {
            if (parse_reg(&ptr, &regA) != PARSE_REG) // parse regA
            {
                err_print("Invalid REG");
                return TYPE_ERR;
            }
            if (parse_delim(&ptr, delim) != PARSE_DELIM) // parse ,
            {
                err_print("Invalid ','");
                return TYPE_ERR;
            }
            if (parse_reg(&ptr, &regB) != PARSE_REG) // parse regB
            {
                err_print("Invalid REG");
                return TYPE_ERR;
            }
        ...
        /*
        case push, pop: 
        parse two registers like rrmov or alu
        check regA is none_reg or not
        */
        case I_PUSHQ:
        case I_POPQ:
        {
        ...
        /*
        case irmov:
        parse two registers and an immediate number
        check regA
        */
        case I_IRMOVQ:
        {
        ...
        /*
        case mrmov or rmmov:
        parse two registers and one of them is memory operands
        */
        case I_MRMOVQ:
        {
            if (parse_mem(&ptr, &value, &regA) != PARSE_MEM) // parse memory
            {
        ...
        case I_RMMOVQ:
        {
            ...
            if (parse_mem(&ptr, &value, &regB) != PARSE_MEM) // parse memory
            {
                err_print("Invalid MEM");
                return TYPE_ERR;
            }
        ...
        /*
        case jXX, call:
        parse imm, sometimes is a symbol
        */
        case I_JMP:
        case I_CALL:
        {
            parse_t type = parse_imm(&ptr, &name, &value);
            if (type == PARSE_DIGIT) //imm: jump or call directly
            {
                for (int i = 0; i < 8; ++i)
                    (line->y64bin).codes[1 + i] = HPACK((value >> (i * 8 + 4)) & 0xF, (value >> (i * 8)) & 0xF);
            }
            else if (type == PARSE_SYMBOL) //symbol: first find the symbol and find the address of the symbol
            {
                add_reloc(name, &(line->y64bin));
            }
            else // parse error
        ...
        /*
        case .byte, .word, .long, .quad, .pos, .align
        */
        case I_DIRECTIVE:
        {
            ...
            case D_DATA:
            {
                parse_t type = parse_data(&ptr, &name, &value);
                if (type == PARSE_DIGIT)
                {
            ...
            case D_ALIGN:
            {
                ...
                int64_t align = (int64_t)value;
                vmaddr = ((vmaddr + align - 1) / align) * align;
                (line->y64bin).addr = vmaddr;
                break;
            }
...
int relocate(void)
{
    ...
    while (rtmp)
    {
        ...
        long value = symtmp->addr;

        for (int i = 0; i < 8; ++i)
        {
            (rtmp->y64bin)->codes[relbytes - 8 + i] = HPACK((value >> (i * 8 + 4)) & 0xF, (value >> (i * 8)) & 0xF);
        }
...
int binfile(FILE *out)
{
    ...
    line_t *line_temp = line_head->next;

    while (line_temp != NULL)
    {
        /* binary write y64 code to output file (NOTE: see fwrite()) */
        if (line_temp->type == TYPE_INS)
        {
            //set the offset by the inst's address
            if (fseek(out, (line_temp->y64bin).addr, SEEK_SET) != 0)
                return -1;

            fwrite((line_temp->y64bin).codes, 1, (line_temp->y64bin).bytes, out);
        }
